...
def read_data(path, label_points):
    ...
    emotions = np.asarray([emotion_dict[y] for y in landmarks.iloc[0].astype(object) if y == "anger" or y == "neutral"], dtype=np.float32)
    print(f"emotions: {emotions}")
    columns = landmarks.shape[1]
    #points = np.zeros((1133, len(label_points) * 2), dtype=np.float32)
    points = []
    for photo in range(columns):
        if landmarks[photo].iloc[0] == "anger" or landmarks[photo].iloc[0] == "neutral":
            #temp_list = [landmarks[photo].iloc[point+1][0:2] for point in label_points]
            #print(temp_list)
            print(f"landmarks: {landmarks[photo]}")
            for point in label_points:
                coords = landmarks[photo].iloc[point+1][0:2]
                points.extend(coords)

    points = np.array(points, dtype=np.float32).reshape(len(emotions), len(label_points*2))
